...
var edgeDistance = 90;//记录相邻图片错开的距离

var imgNow = 0;//记录当前显示的第几张图片，此处为默认值0
var imgMouse = 0;//记录鼠标停留在第几张图片上

var imgOuterWidth = imgOuter.height();//记录所有图片最外层容器的宽度，计算即将显示第一张图片时的，其他图片移动的距离
// alert(imgDiv.size());
/**********方法*************/
...
function autoSlide(){
    ...
    if(imgNow == 0){//显示第一张图片
        ...
        for(var i=imgDiv.length-1;i>0;i--){
            imgDiv.eq(i).animate({'top':imgOuterWidth-(imgDiv.length-i)*edgeDistance+'px'},700);
        }
    }else{//显示第一张之外的任意图片
        //图片左上角的数字样式先改变

        //图片运动
        imgDiv.eq(imgNow).animate({'top':edgeDistance*imgNow+'px'},700);
    }
}
...
function mouseSlide(){
    ...
    if(imgMouse > imgNow){//鼠标左边图片移动，即鼠标选中的是当前图片右边的图片
        for(var i= imgNow+1;i<=imgMouse;i++){
            ...
            //图片移动
            imgDiv.eq(i).stop().animate({'top':edgeDistance*i+'px'},700);
        }
        //重置当前图片的索引
        imgNow = imgMouse;
        ...
        for(var i= imgNow;i>imgMouse;i--){
            //图片左上角数字样式改变
            //图片移动
            imgDiv.eq(i).stop().animate({'top':imgOuterWidth-(imgDiv.length-i)*edgeDistance+'px'},700);
        }
        //重置当前图片的索引
        imgNow = imgMouse;
    }
